...
def main():
    ...
    raw_url = "https://web.archive.org/web/20250416144828id_/https://vocaloid-fantasia.com/cir-iciran06.htm"
    ...
    for table in tables:
        ...
        for row in table_rows[1:]:  # Skip header row
                ...
                if not circle_name:
                    print("Skipping row with empty circle name:", row)
                    continue
                pen_name = sanitize_string(cols[2].get_text())
                circle_urls = []
                web_tag = cols[3].select_one("a")
                if web_tag and web_tag.has_attr("href"):
                    new_url = web_tag["href"].replace("https://web.archive.org/web/20250416144828/", "")
